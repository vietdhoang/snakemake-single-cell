...
class Filter:
    ...
    def _blacklist(get_cutoffs: Callable, adata: AnnData) -> Tuple[list[float]]:
        ...
        obs_bl = obs_bl.union(
            set(
                Filter._query(
                    adata.obs, "total_counts", lower_cutoff=lo, upper_cutoff=hi
                )
            )
        )
        ...
        obs_bl = obs_bl.union(
            set(
                Filter._query(
                    adata.obs, "n_genes_by_counts", lower_cutoff=lo, upper_cutoff=hi
                )
            )
        )
        ...
        obs_bl = obs_bl.union(
            set(
                Filter._query(
                    adata.obs, "pct_counts_mt", lower_cutoff=lo, upper_cutoff=hi
                )
            )
        )
        ...
        var_bl = var_bl.union(
            set(Filter._query(adata.var, "total_counts", lower_cutoff=lo))
        )
        ...
        var_bl = var_bl.union(
            set(Filter._query(adata.var, "n_cells_by_counts", lower_cutoff=lo))
        )
    ...
    def IQR(adata: AnnData, k: float = 1.5) -> AnnData:
        ...
        adata = adata[
            ~adata.obs_names.isin(obs_blacklist), ~adata.var_names.isin(var_blacklist)
        ]
    ...
    def MAD(adata: AnnData, threshold: float = 2.5) -> AnnData:
        ...
        def get_cutoffs(adata: AnnData) -> Tuple[float]:
            return filter_cutoffs.get_mad_cutoffs(adata, threshold=threshold)
    ...
    def zscore(adata: AnnData, threshold: float = 1.96) -> AnnData:
        ...
        def get_cutoffs(adata: AnnData) -> Tuple[float]:
            return filter_cutoffs.get_zscore_cutoffs(adata, threshold=threshold)
...
def main(snakemake) -> None:
    ...
    adata_filtered = Filter(
        adata, snakemake.wildcards.filter_method, **snakemake.params.params
    )
    adata_filtered.run()
