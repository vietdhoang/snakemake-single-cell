...
class Sample:
    ...
    def __init__(self, name: str, data_path: str, 
                 label_path: str, obs_tag: str) -> None:
    ...
    def get_all_samples(cls, config: dict) -> List['__class__']: 
        ...
        for key in config['inputs']:
            ...
            samples.append(
                cls(key, sample['data_path'], 
                    sample['label_path'], sample['obs_tag'])
            )
...
class Method:
    ...
    def get_methods(cls, method_type: str) -> List['__class__']:
        return [
            cls(method_type, method_name) for method_name in config[method_type]
        ]
...
class Tree:
    ...
    @staticmethod
    def get_paths(t: '__class__', 
                  paths: List['__class__'] = None, 
                  current_path: List['__class__'] = None) -> List['__class__']:
        ...
        for method_name in config[method_type]:
            ...
            if len([*method.paramspace.instance_patterns]) == 0:
                ...
                for param_instance in method.paramspace.instance_patterns:
                    ...
                    tree.children.append(
                        Tree(
                            data=(f"{method.method_type}_{method.name}"
                                  f"_{param_instance}")
                        )
                    )
...
def parse_methods(methodorder: List[str] = [], 
                  skip_merged: bool = False,
                  filename: str = 'mtx.h5ad') -> List[str]:
    ...
    for parent_dir in get_samples_basenames(skip_merged=skip_merged):
        for method_param_path in Tree.get_paths(method_tree):
            ...
            output_files.append(
                parent_dir/first_method_type/path/filename
            )
...
def get_plots():
    ...
    if config['plot']['dim_reduce'] and exists('dim_reduce', config):
        ...
        method_tree = make_method_tree(methodorder=['dim_reduce', 
                                                    'filter', 'norm'])        
            ...
            for method_param_path in Tree.get_paths(method_tree):
                ...
                if use_labels:
                    for label in config['labels']:                    
                        output_files.append(
                            parent_dir/'figures'/'dim_reduce'/path/f'scatter_{label}.html'
                        )
    ...
    if config['plot']['cluster'] and exists('cluster', config):
        ...
        method_tree = make_method_tree(methodorder=['cluster', 'filter',
                                                    'norm', 'dim_reduce'])        
        for parent_dir in get_samples_basenames():
            for method_param_path in Tree.get_paths(method_tree):
                path = '/'.join(method_param_path)
        ...
        if 'make-tree' in key:
            ...
            if exists('prior', dict_tmc[key]):                
                if dict_tmc[dict_tmc[key]['prior']]['tmc_qc']:
                    for parent_dir in get_samples_basenames():
                        output_files.append(
                            parent_dir/'too-many-cells'/key/'tmc_filter'/'tmc_norm'/
                    ...
                    for parent_dir in get_samples_basenames():
                        for method_param_path in Tree.get_paths(method_tree):
                            path = '/'.join(method_param_path)
                            output_files.append(
                                parent_dir/'too-many-cells'/key/path/
                                f"{dict_tmc[key]['prior']}.prior.{key}.done"
                            )
            ...
            # If the make-tree doesn't contain a prior
                ...
                if dict_tmc[key]['tmc_qc']:
                    for parent_dir in get_samples_basenames():
                        output_files.append(
                            parent_dir/'too-many-cells'/key/'tmc_filter'/
                            'tmc_norm'/f"{key}.done"
                        )
                else:
                    method_tree = make_method_tree(methodorder=['filter', 'norm'])        
                    for parent_dir in get_samples_basenames():
                        for method_param_path in Tree.get_paths(method_tree):
                            path = '/'.join(method_param_path)
                            output_files.append(
                                parent_dir/'too-many-cells'/key/path/f"{key}.done"
                            )
